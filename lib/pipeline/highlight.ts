...
export async function runHighlight() {
	...
	const prompt = `【最近收集到的新聞標題清單】(index. 標題)\n${articleList}\n\n從上面的清單中,選出對全球讀者而言最重要的最多 ${MAX_HIGHLIGHTS} 則(可以少於 ${MAX_HIGHLIGHTS} 則;不確定的話寧可少選也不要湊數;如果清單裡真的沒有重要新聞,可以選 0 則)。判斷重要性時考慮:是否牽動多國或重大機構、經濟或安全層面的影響程度、是否會有後續連鎖反應。\n\n對每一則入選的新聞,依重要性由高到低給 rank(從 1 開始,不可重複),並執行以下兩件事:\n1. 重新下一個精簡標題(中英文都要)——用自己的話寫,不是逐字複製原文標題,長度跟一般新聞標題差不多,清楚傳達這則新聞在講什麼。\n2. 寫一段導讀(中英文都要)。導讀不是條列名詞解釋,而是像一位懂行的朋友引導讀者理解:先用一兩句話講清楚讀者要看懂這則新聞,必須先掌握的整體情勢或前提脈絡(不要逐一解釋專有名詞,而是講清楚「現在的局面是什麼」);接著說明這則消息本身代表什麼、可能對世界帶來什麼影響或後續效應。整段讀完要能讓讀者明白「這件事為什麼值得我花時間關注」。長度約 5-8 句,語氣自然像在解釋給朋友聽,不要條列、不要生硬翻譯腔。\n\n只回 JSON:{"items":[{"articleIndex":0,"rank":1,"titleZh":"","titleEn":"","whyZh":"","whyEn":""}]}`
	...
	const items = (parsed.items ?? [])
		.filter((item) => articles[item.articleIndex] && item.whyZh && item.whyEn && item.titleZh && item.titleEn)
	...
	await prisma.article.updateMany({
		...
		data: {
			highlightRank: null,
			highlightTitleZh: null,
			highlightTitleEn: null,
			highlightWhyZh: null,
			highlightWhyEn: null,
			highlightedAt: null,
		},
		...
		items.map((item) =>
			prisma.article.update({
				...
				data: {
					...
					highlightTitleZh: toTraditionalZh(item.titleZh ?? ''),
					highlightTitleEn: item.titleEn ?? '',
